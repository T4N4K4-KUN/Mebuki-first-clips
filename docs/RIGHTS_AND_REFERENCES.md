...
INSPIRATION MAP方針では、Bから積極的に参考にする。ただし取り入れる対象は、抽象化した構造、心理トリガー、視点反転、テンポ、学習導線だけに限る。1つの外部作品のカット順、言い回し、図、画面、編集、ジョークをそのままなぞらない。

| Reference | What to learn | Do not do |
|---|---|---|
| Replit Agent | 普通の言葉で頼む、Agentが作る・確認する・直す導線 | UI、コピー、ロゴ、プロンプト例、デモ画面を使わない |
| Bolt.new | 「何を作る？」から始める入口、ビジョン先行の見せ方 | LP文言、画面、テンプレ、ロゴを使わない |
| Lovable | 自然言語からworking applicationへ進む導線 | ドキュメント文、画面、事例を移植しない |
| GitHub Spark | dream -> see -> ship、プレビュー、履歴、共有の流れ | GitHubの表現、サンプルアプリ、画面を使わない |
| Microsoft MakeCode Arcade | 初手からゲームにする、動く画面で掴む | ゲーム名、画面、キャラ、チュートリアル構成を使わない |
| freeCodeCamp | 学習ロードマップ、プロジェクト型学習、初心者向け粒度 | 動画や記事を日本語化して使わない |
| CS50 | Phase 3以降の設計、正しさ、学び直し | 講義、スライド、台詞、演出を使わない |
