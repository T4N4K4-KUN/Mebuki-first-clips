# REFERENCE_TARGETS — steal-map source list

このファイルは、STEAL MAPで「何を盗むか」を実際の参考先に接続するためのリスト。
...
| ID | Source | URL | Class | Steal | Do not steal | Clips |
|---|---|---|---|---|---|---|
| REF-CS50-X | CS50x official course | https://cs50.harvard.edu/x/ | B | 入門を参加儀式にする、難しさを隠さない、Q&Aやツールに頼る導線 | 講義映像、台詞、スライド、課題名、先生の語り口 | 001, 004, 006 |
| REF-CS50-W0 | CS50x Week 0 | https://cs50.harvard.edu/x/weeks/0/ | B | 最初に全体像と参加感を作る、いきなり全部理解させない | Scratch教材、講義構成、字幕、デモ内容 | 001, 003 |
| REF-CS50-W1 | CS50x Week 1 | https://cs50.harvard.edu/x/weeks/1/ | B | CやCLIの怖さを、学びの入口として扱う | Cの説明、コード例、問題セット | 001 |
...
- REF-CS50-W1
- REF-TED-ED-LESSONS
- REF-CRASHCOURSE-CS-SEARCH

Steal: 怖い入口を参加の証拠に変える。赤い画面で3秒止め、問いから反転する。
...
Steal: 量ではなく接触を勝ちにする。5分、1行、1メモの小ささまで落とす。
...
Steal: 過去の中断を弱さではなく地図に変える。恥を下げ、戻る動作へつなげる。
...
Steal: 自力主義を崩す。問いを立て、答えを検証し、次の1手に分解する。
...
Steal: project-first、ここまででOK、文法前に1フレーズ使う。
...
Steal: 継続は毎日やる才能ではなく、戻れる導線を残すこと。
