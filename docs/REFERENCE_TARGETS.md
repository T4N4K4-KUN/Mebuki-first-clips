...
NotebookLM等に入れて精査する動画は、事前にここで選び抜く。
単なるサービス売り込み、ツール機能デモ、アフィリエイト色の強いチュートリアルを大量に入れない。

採用前に必ず確認すること。

1. その動画が教育導入、学習者の心理、作品紹介、または具体的な成功体験を持っているか。
2. 001で使える「画面上の成果」「冒頭3秒」「視聴者の変化」があるか。
3. 公式/非公式を明記できるか。
4. セリフ、秒数、画面で起きていることを確認できるか。
5. サービスの販売文句ではなく、mebukiへ移植できる構造が最低2つあるか。

落とす動画。

- 機能一覧の読み上げが中心。
- ツール名の宣伝が中心。
- 長いアフィリエイト系チュートリアルで、視聴者の変化が弱い。
- 秒数ログを取っても、使える構造が1つしか出ない。
- NotebookLMへ入れる前の時点で、001のツカミに使う根拠が薄い。

001で優先する動画。

- 小さなゲームや作品を冒頭で見せる。
- 既存作品から着想を得て始めることを肯定する。
- ユーザー作品を紹介し、投稿者を称える。
- 動いたあとに中を見る、1か所いじる、再実行する導線がある。
- AIツールの売り込みではなく、視聴者の「自分も触れそう」という心理変化がある。
...
- Replit official clip: 採用。`idea -> software` と `pair programmer, not code generation tool` が有効。
- Bolt/Lovableの長尺チュートリアル: 001では薄い。単純なプロンプトから成果が見える構造だけ採用し、売り込み・DB/API・高度機能は後段へ回す。
- Bolt Discuss: 001では使わない。004/006向け。
- Adafruit Mini Galaga: 採用度高。ユーザー投稿作品、既存ゲームからの着想、動いた後に `edit code` で中を見る、カスタマイズする導線が強い。
